...
function setup() 
{
  ...
  ScoreCount = createSprite(50,20,30,30);;
  ScoreCount.addAnimation("Empty", EmptyStar);
  ScoreCount.addAnimation("One", OneStar);
  ScoreCount.addAnimation("Two", TwoStar);
  ScoreCount.changeAnimation("Empty")
  ScoreCount.scale = 0.2
...
function draw() 
{
  ...
  if(collide(fruit, Star, 20)==true) {
    Star.visible = false;
    if(Score == 1) {
     ScoreCount.changeAnimation("Two") 
    }
    else {
      ScoreCount.changeAnimation("One")
    }
    Score = Score + 1;
  }
